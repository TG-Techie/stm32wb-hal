...
def get_pacakge_name() -> str:
    ...
    with open("Cargo.toml") as cargo_file:
        cargo: dict[str, Any] = toml.load(cargo_file)

    package: dict[str, Any] | None = cargo.get("package")
    assert isinstance(
        package, dict
    ), f"the package in `Cargo.toml` if missing or of the wrong type"

    name: str | None = package.get("name", None)
    assert isinstance(name, str | NoneType)
...
def select_build_to_upload(builds: dict[str, Path] | None = None) -> Path:
    ...
    if file is not None and len(file) > 1:
        error(
            (
                f"ERROR: multiple files specified {file}"
                f"\n    please specify only one file to upload when calling {NAME_OF_THIS_UTIL}"
            )
        )
        ...
        if not file_path.exists() or not file_path.is_file():
            error(
                (
                    f"ERROR: the file to upload does not exist or is not a file {file_path=}"
                    f"\n    please specify a file that exists and is a file when calling {NAME_OF_THIS_UTIL}"
                )
            )
    ...
    if len(builds) == 0:
        error("no build outputs found, please `cargo build` before running")
